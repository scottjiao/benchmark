...
task_to_evaluate=[{"net":"slotGAT","search_num_layers":"[2,3,4]"}]
gpus=["0","1"]
total_trial_num=30
    ...
    for task in task_to_evaluate:
        ...
        for gpu in gpus:
            for _ in range(worker_num):
                command=f"""python run_search.py --feats-type 0 --dataset {dataset} --gpu {gpu} --repeat {re} --trial_num {trial_num} --study_name {study_name} --study_storage {study_storage} --net {task['net']} --search_num_layers "{task['search_num_layers']}" --search_hidden_dim "[32,64]"  """
                p=Run(command)
                p.daemon=True
                p.start()

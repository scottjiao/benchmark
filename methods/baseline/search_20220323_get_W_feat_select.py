...
fixed_info={"get_out":"True","slot_aggregator":"last_fc","selection_weight_average":"True"}
task_to_evaluate=[
    ...
    for task in task_to_evaluate:
        ...
        for root, dirs, files in os.walk("./log", topdown=False):
            for name in files:
                ...
                if ".txt" in name:
                    continue
                if "techniques" not in name:
                    continue
                #auto_feat_selection_lr_times_on_weight
                if "auto_feat_selection_lr_times_on_weight" not in name:
                    continue
        ...
        study_name=f"get_W_{dataset}_net_{task['net']}_auto_feat_selection_lr_times_on_weight_{task['lr_times_on_feat_average']}"

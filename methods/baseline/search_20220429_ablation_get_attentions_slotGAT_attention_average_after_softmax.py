...
for dataset,worker_num,repeat in dataset_to_evaluate:
    for task in task_to_evaluate:
        ...
        yes=["technique","corrected","feat_type",f"feat_type_{task['feats-type']}",f"aggr_{task['slot_aggregator']}","softmax"]
        no=[]
        ...
        study_name=f"get_attentions_{dataset}_net_{task['net']}_feats_type_{task['feats-type']}_slot_aggregator{task['slot_aggregator']}_attention_average_after_softmax"

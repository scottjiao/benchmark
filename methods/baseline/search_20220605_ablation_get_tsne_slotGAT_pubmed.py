...
dataset_to_evaluate=[("pubmed_HNE_complete",1,1),]  # dataset,worker_num,repeat

prefix="ablation";specified_args=["dataset",   "net",    "feats-type",     "slot_aggregator"]


fixed_info={"task_property":prefix,"net":"slotGAT","slot_aggregator":"average","get_out":"True","get_out_tsne":"True"}
task_to_evaluate=[
{"feats-type":"1",}
]
gpus=["1"]
    ...
    for task in task_to_evaluate:
        ...
        for dict_to_add in [task,fixed_info]:
            for k,v in dict_to_add.items():
                args_dict[k]=v
        net=args_dict['net']
        yes=["technique",f"feats-type_{args_dict['feats-type']}",f"slot_aggregator_{args_dict['slot_aggregator']}",]
        no=["oracle"]

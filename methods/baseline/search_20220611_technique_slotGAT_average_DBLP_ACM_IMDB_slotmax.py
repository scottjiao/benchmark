

import time
import subprocess
import multiprocessing
from threading import main_thread
from pipeline_utils import get_best_hypers,run_command_in_parallel,config_study_name,Run
import os

#time.sleep(60*60*4)


dataset_to_evaluate=[("DBLP_corrected",2,10),("ACM_corrected",2,10),("IMDB_corrected",2,10)]  # dataset,worker_num,repeat

prefix="technique";specified_args=["dataset",   "net",    "feats-type",     "slot_aggregator","search_hidden_dim","search_num_layers","predicted_by_slot"]


fixed_info={"task_property":prefix,"net":"slotGAT","slot_aggregator":"average","predicted_by_slot":"max"}
task_to_evaluate=[
{"search_hidden_dim":"[128]","search_num_layers":"[3]","feats-type":0},
{"search_hidden_dim":"[128]","search_num_layers":"[3]","feats-type":1},
{"search_hidden_dim":"[128]","search_num_layers":"[3]","feats-type":2},
{"search_hidden_dim":"[128]","search_num_layers":"[2]","feats-type":0},
{"search_hidden_dim":"[128]","search_num_layers":"[2]","feats-type":1},
{"search_hidden_dim":"[128]","search_num_layers":"[2]","feats-type":2},
{"search_hidden_dim":"[64]","search_num_layers":"[3]","feats-type":0},
{"search_hidden_dim":"[64]","search_num_layers":"[3]","feats-type":1},
{"search_hidden_dim":"[64]","search_num_layers":"[3]","feats-type":2},
{"search_hidden_dim":"[64]","search_num_layers":"[2]","feats-type":0},
{"search_hidden_dim":"[64]","search_num_layers":"[2]","feats-type":1},
{"search_hidden_dim":"[64]","search_num_layers":"[2]","feats-type":2},
]
gpus=["0","1"]
total_trial_num=8


for dataset,worker_num,repeat in dataset_to_evaluate:
    for task in task_to_evaluate:
        args_dict={}
        for dict_to_add in [task,fixed_info]:
            for k,v in dict_to_add.items():
                args_dict[k]=v
        net=args_dict['net']
        ##################################
        ##edit yes and no for filtering!##
        ##################################
        #yes=["technique",f"feat_type_{args_dict['feats-type']}",f"aggr_{args_dict['slot_aggregator']}"]
        #yes=[]
        #no=["attantion_average","attention_average","attention_mse","edge_feat_0","oracle"]
        #best_hypers=get_best_hypers(dataset,net,yes,no)
        #for dict_to_add in [best_hypers]:
        #    for k,v in dict_to_add.items():
        #        args_dict[k]=v
        trial_num=int(total_trial_num/ (len(gpus)*worker_num) )
        if trial_num<=1:
            trial_num=1

        args_dict['dataset']=dataset
        args_dict['trial_num']=trial_num
        args_dict['repeat']=repeat

        study_name,study_storage=config_study_name(prefix=prefix,specified_args=specified_args,extract_dict=args_dict)
        #study_name=f"get_embeddings_{dataset}_net_{task['net']}_feats_type_{task['feats-type']}_slot_aggregator{task['slot_aggregator']}"
        #study_storage=f"sqlite:///db/{study_name}.db"
        
        args_dict['study_name']=study_name
        args_dict['study_storage']=study_storage



        run_command_in_parallel(args_dict,gpus,worker_num)


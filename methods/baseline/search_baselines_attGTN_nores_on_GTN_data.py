    ...
    for task in task_to_evaluate:
        ...
        for gpu in gpus:
            for _ in range(worker_num):
                command=f"""python run_search.py --feats-type 0 --dataset {dataset} --gpu {gpu} --trial_num {trial_num} --study_name {study_name} --study_storage {study_storage} --net {task['net']} --search_num_layers "{task['search_num_layers']}" --search_hidden_dim "[32,64]"    --residual False"""
                p=Run(command)
                p.daemon=True
                p.start()
